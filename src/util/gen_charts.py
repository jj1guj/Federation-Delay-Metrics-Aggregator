...
def generate_charts():
    ...
    data = {
        "last_updated":
        now.replace(tzinfo=datetime.timezone.utc).astimezone(
            timezone('Asia/Tokyo')).strftime('%Y-%m-%d %H:%M:%S'),
        "source_instance":
        config.INSTANCE,
        "charts": {
            "heatmap":
            f"{config.BUCKET_PUBLIC_URL}/{config.PREFIX}/heatmap.png",
            "bar_chart":
            f"{config.BUCKET_PUBLIC_URL}/{config.PREFIX}/chart.png"
        },
        "data": {}
    }
    ...
    for i in range(23, -1, -1):
        ...
        logger.info(
            f"Sorted by diff for time range: {start_time} - {end_time}")
        for instance in sorted_diff:
            logger.info(
                f"Instance: {instance[0]} | {instance[1]} ({instance[2]}-{instance[3]})"
            )
        ...
        for instance in sorted_diff:
            ...
            if instance_host not in instance_data:
                instance_data[instance_host] = {
                    'time_labels': [],
                    'delay_values': []
                }
            time_label = start_time.replace(
                tzinfo=datetime.timezone.utc).astimezone(
                    timezone('Asia/Tokyo')).strftime('%Y-%m-%d %H')
            ...
            if instance[1] not in data["data"]:
                data["data"][instance[1]] = {
                    ...
                    str(instance[1]),
                    "version":
                    f"{instance[2]}-{instance[3]}",
                    "chart_url":
                    str(f"{config.BUCKET_PUBLIC_URL}/{config.PREFIX}/instance/{instance[1]}.png"
                        ),
                    "details": []
                }
            data["data"][instance[1]]["details"].append({
                ...
                str(instance[4]),
                "start_time":
                start_time.replace(tzinfo=datetime.timezone.utc).astimezone(
                    timezone('Asia/Tokyo')).strftime('%Y-%m-%d %H:%M:%S'),
                "end_time":
                end_time.replace(tzinfo=datetime.timezone.utc).astimezone(
                    timezone('Asia/Tokyo')).strftime('%Y-%m-%d %H:%M:%S'),
            })
    ...
    for i in range(24):
        ...
        time_labels.append(
            start_time.replace(tzinfo=datetime.timezone.utc).astimezone(
                timezone('Asia/Tokyo')).strftime('%H'))
        ...
        for instance_name in instances:
            diffs.append(instance_diff_map.get(instance_name, 0)
                         or 0)  # Use 0 if instance not present or diff is None
    ...
    try:
        ...
        im = ax.imshow(heatmap_data, cmap='viridis',
                       aspect='auto')  # Use viridis colormap
        ...
        for i in range(len(time_labels)):
            for j in range(len(instances)):
                text = ax.text(j,
                               i,
                               heatmap_data[i, j].round(1),
                               ha="center",
                               va="center",
                               color="w")  # Display rounded values

        ax.set_xlabel("Instance Host")
        ax.set_ylabel("Time")
        ax.set_title("Federation delays by instance (Heatmap)")
        fig.colorbar(im, ax=ax,
                     label='Average delays (s)')  # Add colorbar with label
    ...
    try:
        ...
        instances = [
            instance[1]
            for instance in sorted_chart_data[now -
        ...
        ax.set_title(
            f"Federation delays by instance for time range: {(now - datetime.timedelta(hours=1)).replace(tzinfo=datetime.timezone.utc).astimezone(timezone('Asia/Tokyo')).strftime('%Y-%m-%d %H')} - {now.replace(tzinfo=datetime.timezone.utc).astimezone(timezone('Asia/Tokyo')).strftime('%Y-%m-%d %H')}"
        )
        plt.tight_layout()
    ...
    try:
        ...
        for instance_host, data in instance_data.items():
            ...
            time_dt = datetime.datetime.strptime(
                data['time_labels'][-1], '%Y-%m-%d %H').replace(minute=0,
                                                                second=0)
            time_dt = jst.localize(time_dt)
            nowtime = datetime.datetime.now(jst).replace(minute=0,
                                                         second=0,
                                                         microsecond=0)
            ...
            ax.set_title(
                f"Federation Delays for {instance_host} (Past 24 Hours)")
            plt.xticks(rotation=45, ha='right')
            # データラベルを表示
            for i, txt in enumerate(data['delay_values']):
                ax.annotate(f"{txt:.1f}s",
                            (data['time_labels'][i], data['delay_values'][i]),
                            textcoords="offset points",
                            xytext=(0, 10),
                            ha='center')
            plt.tight_layout()
            plt.savefig(f"./output/instances/{instance_host}.png",
                        bbox_inches='tight')
            logger.info(
                f"Line chart saved to ./output/instances/{instance_host}.png")
            plt.close(fig)
    except Exception as e:
